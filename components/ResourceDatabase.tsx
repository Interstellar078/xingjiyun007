    ...
    <>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
            ...
            {/* Status Display */}
            {importStatus !== 'idle' && (
                <div className={`flex items-center gap-2 px-3 py-1.5 rounded text-sm animate-fade-in ${
        ...
        <div className="w-56 bg-gray-50 border-r border-gray-200 flex flex-col shrink-0">
          <div className="p-4 border-b border-gray-200 bg-gray-100">
             ...
             <div className="mt-2 relative">
                ...
                <input type="text" placeholder="搜索..." className="w-full pl-7 py-1 text-xs border border-gray-300 rounded" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
             </div>
          </div>
          <div className="flex-1 overflow-y-auto">
            {displayCountries.length > 0 ? (
                ...
                <div key={country} onClick={() => setSelectedCountry(country)} className={`px-4 py-3 cursor-pointer flex justify-between items-center group ${selectedCountry === country ? 'bg-white border-l-4 border-blue-600 shadow-sm' : 'hover:bg-gray-100 border-l-4 border-transparent'}`}>
                    ...
                    {!isReadOnly && <button onClick={(e) => { e.stopPropagation(); handleDeleteCountry(country); }} className="opacity-0 group-hover:opacity-100 text-gray-400 hover:text-red-500"><Trash2 size={12} /></button>}
                </div>
                ))
            ) : (
          ...
          <div className="p-3 border-t bg-white space-y-2">
            {!isReadOnly ? (
                ...
                <div className="text-center text-xs text-gray-400 py-2">
                    <Lock size={12} className="inline mr-1"/> 普通用户无法修改资源库
                </div>
            )}
          </div>
        </div>
                ...
                <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center bg-gray-50">
                    <div>
                        ...
                        <div className="flex gap-4 mt-4">
                            <button onClick={() => setMainTab('poi')} className={`text-sm font-medium pb-1 border-b-2 transition-colors flex items-center gap-2 ${mainTab === 'poi' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
                                <MapPin size={16}/> 地点与资源管理 (城市/机场/区域)
                            </button>
                            <button onClick={() => setMainTab('transport')} className={`text-sm font-medium pb-1 border-b-2 transition-colors flex items-center gap-2 ${mainTab === 'transport' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
                                <Car size={16}/> 交通配置 (国家级)
                            </button>
                        </div>
                    </div>
                        ...
                        <div className="flex-1 p-6 overflow-auto">
                            ...
                            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                                <div className="flex items-center justify-between mb-4">
                                    ...
                                    {!isReadOnly && (
                                        <button onClick={() => docUploadRef.current?.click()} className="text-sm px-3 py-1.5 bg-white border border-gray-300 rounded text-gray-700 hover:bg-gray-100 hover:text-blue-600 flex items-center gap-1 transition-colors">
                                            <Upload size={14}/> 上传文件
                                        </button>
                                    )}
                                </div>
                                ...
                                {currentFiles.length === 0 ? (
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                        {currentFiles.map(file => (
                                            <div key={file.id} className="bg-white border rounded p-3 relative group hover:shadow-md transition-shadow">
